...
folderfile = os.path.join(FOLDER, "folderlocations.log")
ADDON_DIR = xbmc.translatePath(ADDON.getAddonInfo('path'))
putinfolders = os.path.join(ADDON_DIR, 'resources', 'putinfolders')
...
if ADDON.getSetting('reset.addon') == 'true':
    ADDON.setSetting('reset.addon', 'false')
...
def printlogs():
    global message, message1, message2, message3, message4, message5, message6, message7, message8, message9, message10
    messages = []
...
def resetaddon():
    ...
    if loglevel == 'debug':
        printstar()
    if not loglevel == 'none':
        message = 'resetting addon'
        printlog()
        message = 'All addon data and settings will be cleared'
        printlog()
    notifythis = ("%s, resetting addon"% ADDON_ID)
    pause = 'true'
    notifystuff()        
    target = os.path.join(ADDON_DIR, "scripts", "reset.py")
    if loglevel == 'debug':
        message = ("target script is %s"% target)
        printlog()
        printstar()
        ...
        if not newstring == checkstring:
            ...
            if nocentral == 'prompt':
                ...
                if yesnowindow:
                    ...
                    'Central folder selection was unavailable so setting has been disabled'
                    printlog()
...
def getsettings():
    global message, message1, message2, message3, message4, checkfolder, monitorfolder, idletime, freq, loglevel, addonlog, notify, scriptfolderset, scriptfolder, centralfolder, centralfolderset, nocentral, centralidletime, centraldelay, centralfreq, archivenumber, archiveall, notifythis, pause, chk, instructionsfolder
    # set defaults
    ...
    # check for orphan instructions in old folders.
    # notify and log if find old folder
    # if folder not found or instructions not present remove entry in file

    # check for instructions in current folders
    # if not there copy them in and log
    folders = []
    foldersinfile = []
    removefromlist = []
    if os.path.isfile(folderfile):        
        with open(folderfile) as f:
            folders = f.readlines()
            folders = [x.strip() for x in folders]              # clean spaces and line breaks
#            foldersinfile = folders
        with open(folderfile) as f:
            foldersinfile = f.readlines()
            foldersinfile = [x.strip() for x in foldersinfile]



            
    # make sure all current created folders are in list
    if not monitorfolder in folders:
        folders.append(monitorfolder)
    if centralfolderset == 'true':
        if not centralfolder in folders:
            folders.append(centralfolder)
    else:
        centralfolder = 'not set'
    if not workingfolder in folders:
        folders.append(workingfolder)
    if not archive in folders:
    ...
    while c < t:
        chk = folders[c]
        chk = chk.strip()
        if chk == monitorfolder:
            instructionsfolder = os.path.join(putinfolders, 'local')
            checkdefaultfiles()           
        elif chk == centralfolder:
            instructionsfolder = os.path.join(putinfolders, 'central')
            checkdefaultfiles() 
        elif chk == workingfolder:
            checkdefaultfiles() 
        elif chk == archive:
            instructionsfolder = os.path.join(putinfolders, 'archive')
            checkdefaultfiles() 
        else:
            # check for instructions
            chkfile = os.path.join(chk, "aaainstructions.txt")
            if xbmcvfs.exists(chkfile):
                msg = ('Files found in old folder: %s'% chk)
                printlog()
                notifythis = ("%s, processing file"% AddonName)
                pause = 'true'
                notifystuff()
            else:
                removefromlist.append(chk)
                message = ('Removing old folder from list: %s'% check)
                printlog()
        c = c + 1
        
    # check lists, make new folderfile if necessary
    makenewlist = 'false'
    if os.path.isfile(folderfile):
    
        # testing
        message1 = 'check1'
        message2 = ('Contents of folders: %s'% folders)
        message3 = ('Contents of removefromlist: %s'% removefromlist)
        message4 = ('Contents of foldersinfile: %s'% foldersinfile)
        printlogs()
        
        
        newlist = []
        newlist = [x for x in folders if x not in removefromlist]
        
        # testing
        message = ('Contents of newlist: %s'% newlist)
        printlog()
        
        if not newlist == foldersinfile:
            makenewlist = 'true'
    else:
    ...
    # testing 
    message = ('makenewlist is: %s'% makenewlist)
    printlog()    
    ...
    if makenewlist == 'true':
        ...
        with open(folderfile, "w") as myfile:
            ...
            # testing
            message1 = 'Writing newlist to folderfile'  
            message2 = ('folderfile is: %s'% folderfile)
            message3 = ('newlist is: %s'% newlist)
            printlogs()
...
def checkdefaultfiles(): 
    ...
    wanted = []
    existing = []
    wanted = os.listdir(instructionsfolder)
    # check for "/" at end - for xbmcvfs, needed for directories
    last = chk[-1:]
    if not last == "/":
        foldertocheck = chk + "/"
        ...
        foldertocheck = chk
    if xbmcvfs.exists(foldertocheck):
        dirs, existing = xbmcvfs.listdir(foldertocheck)
        num = len(wanted)
        c = 0
        while c < num:
            filename = wanted[c]
            if not filename in existing:
                source = os.path.join(instructionsfolder, filename)
                target = os.path.join(foldertocheck, filename)     
                # testing
                printstar()
                message = ('Source is %s'% source)
                printlog()
                message = ('Target is %s'% target)
                printlog()
                printstar() 
                try:
                    # shutil.copyfile(source, target)
                    xbmcvfs.copy(source, target)
                    message = ('Copied %s from %s to %s folder'% (filename, instructionsfolder, foldertocheck))
                    printlog()
                except:
                    message = ('Problem.  Failed to copy %s from %s to %s folder'% (filename, instructionsfolder, foldertocheck))
                    printlog()   
                    notifythis = ("%s, problem - check log"% AddonName)
                    notifystuff()
            c = c + 1
    else:
        message=('Could not access folder: %s'% foldertocheck)
        printlog()
...
def runchecksanddostuff():
    ...
    if monitorfolder == workingfolder:
        ...
        if size == 0:
            ...
            message = ('Checking local folder: %s'% monitorfolder)
            printlog()
            message = ('%d files found in the local folder to move: %s'% (size, interesting))
            printlog()
        if size == 0:
            checkworking()
        else:
...
xbmc.sleep(10000)
if ADDON.getSetting('notify.startup') == 'true': 
