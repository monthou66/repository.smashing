...
def printlog():
    global message, printaddonlog
    if not loglevel == 'off':
        xbmc.log("%s:    %s"% (ADDON_ID, message), 2)
    if addonlog == 'true':
        if printaddonlog == 'true':
            printaddonlog()
    message = ""
    printaddonlog = 'false'
    
def printlogs():
    global message, message2, message3, message4, message5, message6, message7, message8, message9, message10, nextmess, printaddonlog
    messages = []
...
def notifystuff():
    global notifythis, pause, message, printaddonlog
    if notify == 'true':
        if not notifythis == "":
...
def stopaddon():
    global message, notifythis, pause, printaddonlog
    if loglevel == 'debug':
        printstar()
...
def checkfolderpath():
    global monitorfolder, monitorfile, message,printaddonlog
