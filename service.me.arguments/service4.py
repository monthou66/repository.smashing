...
if  not os.path.isdir(tempfolder):
    os.mkdir(tempfolder)
nodelaytemp = os.path.join(tempfolder, "nodelay")
...
def checkcentralfolder():
    ...
    # process files - copy to local temp folder, add host to filename
    c = 0
    while c < textnum:
        name = textfiles[c]
        centralfile = os.path.join(centralfolder, name)
        if 'nodelay' in name:
            localfile = os.path.join(nodelaytemp, name)
        else:
            seconds = time.time()       # seconds past epoch for timestamp
            seconds = int(seconds)      # > whole number
            add = int(centraldelay)
            actiontime = seconds + add
            act = str(actiontime)
            delayname = act + name

        
            localfile = os.path.join(tempfolder, delayname)
        namelesstxt = name[:-3]
        newname = namelesstxt + host + '.txt'
        newcentralfile = os.path.join(centralfolder, newname)
        
        # check localfile and newcentralfile don't already exist.
        ...
        message = ('File is %s'% name)
        printlog()
        if not localfile == 'not set':
